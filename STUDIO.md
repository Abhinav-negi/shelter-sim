...
| F2b | Walls as single extruded solids (residual seams) | F2 | | DONE | Each wall now one `ExtrudeGeometry` (rectangle `Shape` + window `Path` hole), 4 walls merged+vertex-welded into one mesh (`Building.tsx`). Visual QA found a real defect beyond the old per-window seams: a coplanar-polygon rasterization crack at the S/E-type corners (S/N wall's exposed end-grain vs the trimmed E/W wall's front face) under SwiftShader — root-caused by bisection, fixed with a documented sub-mm geometry nudge (`CORNER_OVERLAP_EPS`/`CORNER_DEPTH_EPS`) plus `deviceScaleFactor:2` screenshot capture. All 4 required views clean at normal scale; former-seam-column pixel deltas now single-digit. `npm run build -w @shelter/studio` clean; `npm test -w @shelter/studio` 38/38; frozen-path diff empty. See `ledger/tasks/F2b.md` Evidence. |
