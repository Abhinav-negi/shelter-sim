// Port of apps/client/src/features/results/explain.ts (logic only, not the UI —
// F3.md's ## Read). Turns raw KPIs into up to three plain-language sentences a
// non-expert can read in ~10 seconds, each tagged with a severity. Pure
// function, no invented metrics (F3.md condition 4): every number here is a
// SimulationKpis field, just phrased in words.
import type { SimulationKpis } from '@shelter/engine';
import { kToC } from './format';
...
export function explain(kpis: SimulationKpis): Insight[] {
  ...
  insights.push({
    ...
    text: `The walls smooth out ${((1 - kpis.decrementFactor) * 100).toFixed(0)}% of the outdoor temperature swing (decrement factor ${kpis.decrementFactor.toFixed(2)}, lower is better) — ${
      kpis.decrementFactor < 0.4
        ? 'strong thermal mass at work'
        : 'more insulation or mass would help'
    }.`,
  });
