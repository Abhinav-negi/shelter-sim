...
**Why this exists.** _"This is where orientation stops being an abstraction. You watch the
...
principle by itself."_ (`plan.md` §6.) Orientation is a word the problem statement uses by name.
...
2. **On a Leh January day the south bar is the largest of the four walls and the north bar is
   effectively zero.** Paste all five bar values in kWh. _(A non-zero north bar in winter means beam
   is leaking onto a north wall — a finding for T-14, not a chart fix.)_
