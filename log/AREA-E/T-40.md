...
- `Worker.terminate()` on a **busy** worker (mid synchronous `simulate()`) was observed on this
  ...
  out at the default 5s). Fixed by treating _any_ `exit` while the pool is not `destroyed` as
...
from "key absent," which an _optional_ (`?:`) property promises never to be. `npx vitest run`
