...
>   auxKWhPerYear: number;
>   keroseneLitresPerYear: number;
>   co2KgPerYear: number;
>   costINRPerYear: number;
>   method: string;
> };
> ```
>
...
10. **The sunless-streak scenario shows monotonic decline:** its `tempAt0600` on the final day is
    **lower** than on the first day. Paste the per-day series. _(If it does not decline, either the
    multi-day run is restarting each day or the storage physics is wrong — report which.)_
