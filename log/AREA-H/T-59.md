...
**Why this exists.** _"We do not ask 'how does this design do on a typical day?' We ask 'how does
this design do on every day that matters?'"_ This is the strongest thing in the pitch, and it rests
...
>   days: number; // days > 1 only for the sunless streak
...
> the annual fuel total honest: **never scale a single January day by 365.** 13. **The coldest day on record** — the 24 h window with the lowest mean temperature. 14. **The hottest day on record** — because a shelter optimised only for winter can bake in July,
> and we should catch that. 15. **The 1-in-100 design winter day** — the **99th-percentile cold** day, the cold-but-not-freak
...
> `sourceNote`. 16. **The longest sunless stretch** — the longest run of consecutive days whose daily GHI total is
...
> actual streak length. 17. **The clear cold night** — the **coldest night that was also cloudless**. Counter-intuitively
...
> documented clear-sky threshold. 18. **The annual-mean day** — the reference against which the other seventeen are read.
>
...
7. **The clear cold night is colder in effective sky terms than the coldest night overall:** its
   ...
   _(This is the whole reason the scenario exists — if it is not worse, the selection rule is wrong.)_
